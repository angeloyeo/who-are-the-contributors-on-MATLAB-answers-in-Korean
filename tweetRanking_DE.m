...
try
    ...
    while checkdate > datetime - howfar2check
        page = page + 1;
        xDoc = xmlread(['https://de.mathworks.com/matlabcentral/answers' ...
            '/questions?language=de&format=atom&sort=updated+desc&status=answered' ...
            '&page=' num2str(page)]);
        % まず各投稿は <entry></entry>
...
status = "Diejenigen, die MATLAB Answers-Fragen auf Deutsch beantwortet haben (wöchentliches Ranking) " + newline + newline;

for ii=1:min(height(dataset),5)
    status = status + "- " + dataset.nickname(ii) + newline;
end
status = status + newline + "Vielen Dank!" + newline;
status = status + "Siehe Einzelheiten："  + baseURL;
disp(status);

if tweet
    try
        % webwrite(tturl, 'api_key', api_key, 'status', status, options);
        py.tweetDEAnswers.tweetV2(status)
    catch ME
        disp(ME)
    end
end
