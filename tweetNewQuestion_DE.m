...
try
    ...
    xDoc = xmlread('https://de.mathworks.com/matlabcentral/answers/questions?language=de&format=atom&sort=asked+desc');
...
latestID = readmatrix('latestID_DE.txt');
...
if sum(newID) == 0
    ...
    % save
    writematrix(LatestID_JP,'latestID_DE.txt')
    ...
    for ii=1:sum(newID)
        ...
        if thisAuthor == "MathWorks Support Team"
            status = "Ein Hinweis von " + thisAuthor + " :「" + thisTitle + "」 "  + thisURL;
        else
            status = "Eine Frage von " + thisAuthor + " :「" + thisTitle + "」 "  + thisURL;
        end
        disp(status);
        try
